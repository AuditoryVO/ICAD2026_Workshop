...
# Load SoundFont file
sfid = synth.sfload("SoniAladin/Audio/FluidR3_GM2-2.SF2")

# Define MIDI channels and GM Program numbers
CH_PIANO = 0
CH_STRINGS = 1

PIANO_PROGRAM = 0      # General MIDI standard for Acoustic Grand Piano
PIZZICATO_PROGRAM = 45 # General MIDI standard for Pizzicato Strings

# Assign instruments to channels
synth.program_select(CH_PIANO, sfid, 0, PIANO_PROGRAM)
synth.program_select(CH_STRINGS, sfid, 0, PIZZICATO_PROGRAM)



# Load files used as auditory cues
...
while running:
    try:
        ...
        if msg == "Sonification":
            ...
            with midiout:
                synth.start()  # Starts real-time audio playback driver
                        ...
                        for x in range(x_dim):
                            if (x % resolution == 0):
                                ...
                                if normalized_brights[y][x] >= high_threshold: #Stars and Galaxies
                                    ...
                                    print("_________Piano note:", note)
                                    print("_________Piano amplitude:", amplitude)
                                    ...
                                    synth.noteon(CH_PIANO, note, amplitude)
            
                                if low_threshold < normalized_brights[y][x] < high_threshold:    #Faint objects
                                    print("_________Strings note:", note)
                                    print("_________Strings amplitude:", amplitude)
                                    ...
                                    midiout.send_message(note_on)
                                        ##midiout.send_message(note_off)
                                    synth.noteon(CH_STRINGS, note, amplitude)
